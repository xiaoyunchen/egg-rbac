...
class rbac {
  ...
  constructor(app) {
    ...
    const conn = app.mongooseDB.get(app.config.rbac.dbName)

    this.storage = new mongooseStorage(this.mongoose, conn);
  ...
  newRole({ name, alias, grants, desc }) {
    ...
    debug('new role name %s alias %s', name, alias, desc);
    return this.storage.newRole({ name, alias, grants, desc });
  }
  ...
  newPermission({ name, alias, type, desc }) {
    ...
    if (!type) {
      return new Error('[egg-rbac] newPermission parameter type is undefined');
    }
    debug('new permission name %s alias %s', type, name, alias, desc);
    return this.storage.newPermission({ name, alias, type, desc });
  }
  ...
  modifyRole(_id, info) {
    ...
    if (!info.alias && !info.desc) {
      return new Error('[egg-rbac] modifyRoleAlias parameter desc is undefined');
    }
    return this.storage.modifyRole(_id, info);
  }
  ...
  modifyPermission(_id, info) {
    ...
    if (!info.alias && !info.desc) {
      return new Error('[egg-rbac] modifyPermissionAlias parameter info is undefined');
    }
    return this.storage.modifyPermission(_id, info);
  }
  ...
  /**
   * @method rbac#getAllPermissionByType
   * @return {object} promise
   */
  getAllPermissionByType(type) {
    return this.storage.Permission.find({ type });
  }
  /**
   * @method rbac#getRole
   * @param {string} name role name
  ...
  getRoleById(id) {
    if (!id || typeof id !== 'string') {
      return new Error('[egg-rbac] getRoleById parameter id must string');
    }
